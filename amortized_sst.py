...
def train_amortized_sst(sst_model, mf_model, males, females, epochs=20, device='cuda'):
    ...
    user_ids = torch.cat([torch.tensor(males), torch.tensor(females)]).to(device)
    labels = torch.cat([torch.zeros(len(males)), torch.ones(len(females))]).to(device)
    
    # empirical ratio
    hat_p1 = len(females) / (len(males) + len(females))
    ...
    for _ in range(epochs):
        ...
        # Weight for class 1 (female): p1 / hat_p1
        # Weight for class 0 (male): (1-p1) / (1-hat_p1)
